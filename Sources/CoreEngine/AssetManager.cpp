...
//#TODO use OnPause and OnResume to load and unload resources
...
smart_pointer<Texture2D> AssetManager::LoadTextureAtlas(const char* filename,
	int* regions,
	int textures_count,
	...
	GLenum dataType)
{
	...
	smart_pointer<Texture2D> txt = AssetManager::LoadTexture(filename, generateMipmaps, custom, internalFormat, format, dataType);
...
smart_pointer<Texture2D> AssetManager::LoadTextureTiled(const char* filename,
	int rows,
	int columns,
	int tilesCount,
	bool generateMipmaps,
...
smart_pointer<Texture2D> AssetManager::LoadTexture(const char* filename,
	bool generateMipmaps,
	bool custom,
	GLenum internalFormat,
	GLenum format,
	GLenum dataType)
{
	...
	{
		Logger::PrintError("Error occurred while loading image file %s\n", filename);
...
smart_pointer<Material> AssetManager::LoadMaterial(const char* name, const char* vertexShaderRelativePath, const char* fragmentShaderRelativePath)
{
